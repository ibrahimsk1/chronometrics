...
type ClickHouseConfig struct {
	...
	// Retry backoff for initial connection
	MaxRetries     int
...
func Load() (Config, error) {
	...
	if v, ok := envAny("CHRONOMETRICS_CLICKHOUSE_MAX_OPEN_CONNS"); ok && v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			cfg.ClickHouse.MaxOpenConns = n
		}
	}
	if v, ok := envAny("CHRONOMETRICS_CLICKHOUSE_MAX_IDLE_CONNS"); ok && v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			cfg.ClickHouse.MaxIdleConns = n
		}
	}
	if v, ok := envAny("CHRONOMETRICS_CLICKHOUSE_CONN_MAX_LIFETIME"); ok && v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.ClickHouse.ConnMaxLifetime = d
		}
	}
	if v, ok := envAny("CHRONOMETRICS_CLICKHOUSE_MAX_RETRIES"); ok && v != "" {
		if n, err := strconv.Atoi(v); err == nil {
			cfg.ClickHouse.MaxRetries = n
		}
	}
	if v, ok := envAny("CHRONOMETRICS_CLICKHOUSE_RETRY_BASE_DELAY"); ok && v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.ClickHouse.RetryBaseDelay = d
		}
	}
	if v, ok := envAny("CHRONOMETRICS_CLICKHOUSE_RETRY_MAX_DELAY"); ok && v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.ClickHouse.RetryMaxDelay = d
		}
	}
